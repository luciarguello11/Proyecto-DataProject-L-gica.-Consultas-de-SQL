...
from "film"
cross join "category"; 
-- No aporta valor, su utilidad no es práctica dado que genera un producto cartesiano, combinando todas las filas
-- de la tabla film con todas y cada una de las filas de la tabla category, este resultado nos da relaciones inexistentes en la realidad.
...
select "id_cliente","nombre","apellido", count("id_alquiler") as "alquileres_hechos"
...
WHERE fc.category_id IN (SELECT category_id FROM categoria_objetivo);
